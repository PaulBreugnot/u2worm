...
species EnzymeProducer {
	...
	// Concentration at which the reaction rate from [C_gl] to [C_gl_dom] is half of the limiting reaction rate.
	float K_C_gl <- 0.0;
...
species SimulatedAnnealingState {
	Enzymes enzymes;
	float X_C_gl_labile_to_dom <- 0.0;
	...
	float X_C_gl_recal_to_dom <- 0.0;
	float X_C_gl_recal_to_labile <- 0.0;
	...
	action _init(
		...
		if(total_C_labile > 0.0) {
			...
			float E_C_gl_labile <- min(
				(enzyme_producer.L_R_enzyme_rate * C_microbes * dt * enzymes.T_C_gl) * total_C_labile / (enzyme_producer.K_C_gl + total_C_labile),
				total_C_labile
			);
			...
			X_C_gl_labile_to_dom <- E_C_gl_labile - enzyme_producer.beta_C * E_C_gl_labile * E_C_om_labile / total_C_labile;
			X_C_om_labile_to_dom <- E_C_om_labile - (1-enzyme_producer.beta_C) * E_C_gl_labile * E_C_om_labile / total_C_labile;
		...
		float X_C_gl_recal <- 0.0;
		float X_C_om_recal <- 0.0;
		if (total_C_recal > 0.0) {
			// Recalcitrant C decomposition
			float E_C_gl_recal <- min(
				((1-enzyme_producer.L_R_enzyme_rate) * C_microbes * dt * enzymes.T_C_gl) * total_C_recal / (enzyme_producer.K_C_gl + total_C_recal),
				total_C_recal
			);
			...
			X_C_gl_recal <- E_C_gl_recal - enzyme_producer.beta_C * E_C_gl_recal * E_C_om_recal / total_C_recal;
			X_C_om_recal <- E_C_gl_recal - (1-enzyme_producer.beta_C) * E_C_gl_recal * E_C_om_recal / total_C_recal;
		...
		X_C_gl_recal_to_dom <- enzyme_producer.gama_C_gl * X_C_gl_recal;
		X_C_gl_recal_to_labile <- X_C_gl_recal - X_C_gl_recal_to_dom;
	...
	float C_avail(float C_DOM_gl, float C_DOM) {
		return C_DOM_gl + C_DOM + X_C_gl_labile_to_dom + X_C_om_labile_to_dom + X_C_gl_recal_to_dom + X_C_om_recal_to_dom;
	}
...
//		;
	...
	SimulatedAnnealingState neighbour {
		...
		list<float> _T;
		ask enzyme_producer {
			float T_min <- x_C_gl * min_enzymes.T_C_gl + x_C_om * min_enzymes.T_C_om + x_P * min_enzymes.T_P + x_N * min_enzymes.T_N;
			T_range <- T_max - T_min;
			_T <- [
				x_C_gl * (myself.s.enzymes.T_C_gl - min_enzymes.T_C_gl)/T_range,
...
//			_T[i] <- rnd(max(0, _T[i] - delta), min(range, _T[i] + delta));
		...
		ask enzyme_producer {
			create Enzymes with: [
				T_C_gl: min_enzymes.T_C_gl + _T[0]*T_range / x_C_gl,
...
//		ask enzyme_producer {
//			create Enzymes with: [
//				T_C_gl: min_enzymes.T_C_gl + t[0]*T_range / x_C_gl,
...
//		}
	...
	float E(
		...
		if (N_avail > 0 and P_avail > 0) {
			...
			float max_C_decomposition <- enzyme_producer.T_max / min(enzyme_producer.x_C_gl, enzyme_producer.x_C_om) * dt * C_microbes;
			
			ask state {
				result <-
				(1.0 - C_avail / N_avail / myself.C_N) ^ 2 +
				(1.0 - C_avail / P_avail / myself.C_P) ^ 2
				+
				(1.0 - (X_C_gl_labile_to_dom + X_C_om_labile_to_dom + X_C_gl_recal_to_dom + X_C_gl_recal_to_labile + X_C_om_recal_to_dom + X_C_om_recal_to_labile)
					/ max_C_decomposition
				) ^ 2
				;
			}
...
experiment TestSimulatedAnnealing type: gui {
	...
	reflex {
		ask test_simulated_annealing {
			...
			write "";
			write "s: " + s;
			write "T_C_gl: " + s.enzymes.T_C_gl / (#gram / #h);
